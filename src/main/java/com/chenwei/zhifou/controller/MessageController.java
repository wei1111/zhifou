...
public class MessageController {
    ...
    public String conversationDetail(Model model) {
        try {
            ...
            for (Message msg : conversationList) {
                ...
                vo.set("user", user);
                vo.set("unread", messageService
                        .getConvesationUnreadCount(localUserId, msg.getConversationId()));
    ...
    public String conversationDetail(Model model, @Param("conversationId") String conversationId) {
        try {
            List<Message> conversationList =
                    messageService.getConversationDetail(conversationId, 0, 10);
    ...
    public String addMessage(@RequestParam("toName") String toName,
        ...
        try {
            if (hostHolder.getUser() == null) {
                return FJsonUtil.getJSONString(999, "未登录");
            }
            User user = userService.selectByName(toName);
            if (user == null) {
                return FJsonUtil.getJSONString(1, "用户不存在");
            }
            ...
            return FJsonUtil.getJSONString(1, "插入站内信失败");
        }
    }
    ...
    public String addMessage(@RequestParam("fromId") int fromId,
        ...
        try {
            ...
            //msg.setConversationId(fromId < toId ? String.format("%d_%d", fromId, toId) : String.format("%d_%d", toId, fromId));
            messageService.addMessage(msg);
            return FJsonUtil.getJSONString(msg.getId());
        } catch (Exception e) {
            logger.error("增加评论失败" + e.getMessage());
            return FJsonUtil.getJSONString(1, "插入评论失败");
        }
    }
}
